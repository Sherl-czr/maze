...
class Player(pygame.sprite.Sprite):
    def __init__(self, center_x, center_y, forward_angle):
        ...
        # move and rotate param
        self.move_velocity_limit = 1000  # 移动速度的上限
        self.move_velocity = 0  # 当前的移动速度
        self.move_acc = 150  # 每秒将速度增加600个像素
        self.rotate_velocity_limit = 100  # 角速度上限
    ...
    def move(self, direction=1):
        if direction == 1 and abs(self.move_velocity) > 55:
    ...
    def crash(self):
        ...
        if self.move_velocity >= 0:
            self.move_velocity = min(-self.move_velocity * 0.5, -80)
        else:
            self.move_velocity = max(self.move_velocity * 0.5, 80)

        vx = self.move_velocity * math.cos(math.radians(self.forward_angle))
        vy = self.move_velocity * math.sin(math.radians(self.forward_angle))
        new_x = self.rect.x + vx * self.delta_time
        new_y = self.rect.y + vy * self.delta_time
        self.rect.x = new_x
        self.rect.y = new_y
        if pygame.sprite.spritecollide(self, pygame.sprite.Group(), False, collided_rect):
            self.rect = old_rect
            self.rotate_velocity = 0
        # self.rect.x -= vx * self.delta_time
        # self.rect.y -= vy * self.delta_time
